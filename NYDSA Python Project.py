...
df_net1=df_net.rename(columns={'Series or Movie':'Types'})
df_net1
...
netflix_2021['Genre'] = netflix_2021['Genre'].fillna(netflix_2021['Genre'].mode()[0])
netflix_2021['Languages'] = netflix_2021['Languages'].fillna(netflix_2021['Languages'].mode()[0])

#Hidden Gem Score. Replace null values with the mean of the ratings as there are few missing ratings

netflix_2021['Hidden Gem Score'] = netflix_2021['Hidden Gem Score'].fillna(netflix_2021['Hidden Gem Score'].mean())

# Country Availability could provide value insights into Netflix's coverage so replace missing values by "not known"
netflix_2021['Country Availability'].replace(np.nan, 'Not Known',inplace  = True)
...
'''
#Drop NA's
netflix_2021.dropna(inplace=True)

#Drop any duplicates in the data frama
netflix_2021.drop_duplicates(inplace= True)

#Drop the irrelevant columns from the data set:
netflix_2021.drop(labels=['Tags', 'Writer', 'Production House', 'Netflix Link', 'IMDb Link', 'TMDb Trailer', 'Trailer Site'  ], axis = 1)
...
#Drop the irrelevant columns from the data set:
netflix_2021.drop(columns= ['Tags', 'Writer', 'Production House', 'Netflix Link', 'IMDb Link', 'TMDb Trailer', 'Trailer Site'])


# In[12]:


netflix_2021.isnull().sum()


# In[20]:


netflix_2021.isna().sum()
